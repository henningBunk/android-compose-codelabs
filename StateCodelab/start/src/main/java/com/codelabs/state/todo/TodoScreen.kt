...
fun TodoItemInlineEditor(
    ...
    text = item.task,
    onSetText = { onEditItemChange(item.copy(task = it)) },
    icon = item.icon,
    onSetIcon = { onEditItemChange(item.copy(icon = it)) },
    submit = onEditDone,
    iconsVisible = true
) {
    Row {
...
fun TodoItemInput(
    ...
    Column {
        ...
        if (iconsVisible) {
            AnimatedIconRow(icon = icon, onIconChange = onSetIcon, Modifier.padding(top = 8.dp))
        } else {
